...
class NextbotConfigGUI:
    def __init__(self, master):
        ...
        # Create GUI elements to prompt for inputs
        tk.Label(master, text="Name: ").grid(row=0, column=0)
        tk.Entry(master, textvariable=self.name).grid(row=0,column=1)

        tk.Label(master, text="Texture:").grid(row=0, column=0)
        tk.Entry(master, textvariable=self.texture).grid(row=0, column=1)
        tk.Button(master, text="Select", command=self.choose_texture_file).grid(row=0, column=2)

        tk.Label(master, text="Chase Sound:").grid(row=1, column=0)
        tk.Entry(master, textvariable=self.chase_sound).grid(row=1, column=1)
        tk.Button(master, text="Select", command=self.choose_chase_sound_file).grid(row=1, column=2)

        tk.Label(master, text="Death Sound:").grid(row=2, column=0)
        tk.Entry(master, textvariable=self.death_sound).grid(row=2, column=1)
        tk.Button(master, text="Select", command=self.choose_death_sound_file).grid(row=2, column=2)

        tk.Label(master, text="Death Texture:").grid(row=3, column=0)
        tk.Entry(master, textvariable=self.death_texture).grid(row=3, column=1)
        tk.Button(master, text="Select", command=self.choose_death_texture_file).grid(row=3, column=2)

        tk.Label(master, text="Chase Speed:").grid(row=4, column=0)
        tk.Entry(master, textvariable=self.chase_speed).grid(row=4, column=1)

        tk.Label(master, text="Wonder Speed:").grid(row=5, column=0)
        tk.Entry(master, textvariable=self.wonder_speed).grid(row=5, column=1)

        tk.Button(master, text="Create Nextbot", command=self.create_nextbot).grid(row=6, column=1)

        tk.Button(master, text="Reset", command=self.reset_inputs).grid(row=6, column=2)
    ...
    def create_nextbot(self):
        ...
        with open("nextbot_configs.txt", "a") as f:
